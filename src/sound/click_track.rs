//!
//! object to provide a click track on beat changes
//! 

pub struct ClickTrack {
    gain: f32,
    beat: u8,
    tic: Vec<f32>,
    toc: Vec<f32>,
    idx: usize,
    mute: bool,
}

const CLICK_SIZE: usize = 12000;
const MUCLIP: f32 = 32635.0;
const ULAW_DECODE: [i32; 256] = [
    -32124, -31100, -30076, -29052, -28028, -27004, -25980, -24956,
    -23932, -22908, -21884, -20860, -19836, -18812, -17788, -16764,
    -15996, -15484, -14972, -14460, -13948, -13436, -12924, -12412,
    -11900, -11388, -10876, -10364,  -9852,  -9340,  -8828,  -8316,
     -7932,  -7676,  -7420,  -7164,  -6908,  -6652,  -6396,  -6140,
     -5884,  -5628,  -5372,  -5116,  -4860,  -4604,  -4348,  -4092,
     -3900,  -3772,  -3644,  -3516,  -3388,  -3260,  -3132,  -3004,
     -2876,  -2748,  -2620,  -2492,  -2364,  -2236,  -2108,  -1980,
     -1884,  -1820,  -1756,  -1692,  -1628,  -1564,  -1500,  -1436,
     -1372,  -1308,  -1244,  -1180,  -1116,  -1052,   -988,   -924,
      -876,   -844,   -812,   -780,   -748,   -716,   -684,   -652,
      -620,   -588,   -556,   -524,   -492,   -460,   -428,   -396,
      -372,   -356,   -340,   -324,   -308,   -292,   -276,   -260,
      -244,   -228,   -212,   -196,   -180,   -164,   -148,   -132,
      -120,   -112,   -104,    -96,    -88,    -80,    -72,    -64,
       -56,    -48,    -40,    -32,    -24,    -16,     -8,      0,
     32124,  31100,  30076,  29052,  28028,  27004,  25980,  24956,
     23932,  22908,  21884,  20860,  19836,  18812,  17788,  16764,
     15996,  15484,  14972,  14460,  13948,  13436,  12924,  12412,
     11900,  11388,  10876,  10364,   9852,   9340,   8828,   8316,
      7932,   7676,   7420,   7164,   6908,   6652,   6396,   6140,
      5884,   5628,   5372,   5116,   4860,   4604,   4348,   4092,
      3900,   3772,   3644,   3516,   3388,   3260,   3132,   3004,
      2876,   2748,   2620,   2492,   2364,   2236,   2108,   1980,
      1884,   1820,   1756,   1692,   1628,   1564,   1500,   1436,
      1372,   1308,   1244,   1180,   1116,   1052,    988,    924,
       876,    844,    812,    780,    748,    716,    684,    652,
       620,    588,    556,    524,    492,    460,    428,    396,
       372,    356,    340,    324,    308,    292,    276,    260,
       244,    228,    212,    196,    180,    164,    148,    132,
       120,    112,    104,     96,     88,     80,     72,     64,
	    56,     48,     40,     32,     24,     16,      8,      0 
];


impl ClickTrack {
    pub fn new() -> ClickTrack {
        let mut rval = ClickTrack {
            gain: 1.0,
            mute: true,
            beat: 0,
            tic: vec![],
            toc: vec![],
            idx: 0,
        };
        let tic_mu = include_bytes!("../clicks/tic.raw");
        for &i in tic_mu {
            rval.tic.push(1.5 * ULAW_DECODE[i as usize] as f32 / MUCLIP);
        }
        let toc_mu = include_bytes!("../clicks/toc.raw");
        for &i in toc_mu {
            rval.toc.push(1.5 * ULAW_DECODE[i as usize] as f32 / MUCLIP);
        }
        rval
    }
    pub fn get_gain(&self) -> f64 {
        self.gain as f64
    }
    pub fn get_mute(&self) -> bool {
        self.mute
    }
    pub fn set_gain(&mut self, gain: f64) -> () {
        self.gain = gain as f32;
    }
    pub fn set_mute(&mut self, mute: bool) -> () {
        self.mute = mute;
    }
    pub fn mix_into(&mut self, beat: u8, out_a: &mut [f32], out_b: &mut [f32]) -> () {
        // this is where we mix in the click
        if beat != self.beat {
            self.beat = beat;
            self.idx = 0;
        }
        if !self.mute && self.idx < CLICK_SIZE - out_a.len() {
            let mut i: usize = 0;
            while i < out_a.len() {
                if beat == 0 {
                    out_a[i] += self.gain * self.tic[self.idx+i];
                    out_b[i] += self.gain * self.tic[self.idx+i];
                } else {
                    out_a[i] += self.gain * self.toc[self.idx+i];
                    out_b[i] += self.gain * self.toc[self.idx+i];
                }
                i += 1;
            }
            self.idx += out_a.len();
        }
    }
}
